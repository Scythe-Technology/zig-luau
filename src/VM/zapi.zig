...
fn handleError(L: *lua.State, err: anyerror) noreturn {
    switch (err) {
        ...
        else => L.LerrorL("{s}", .{@errorName(err)}) catch ldo.throw(L, .ErrMem),
    }
}
...
pub fn Zyielderror(L: *lua.State) anyerror {
    try L.pushlstring("attempt to yield across metamethod/C-call boundary");
...
pub fn Zcallmeta(L: *lua.State, obj: i32, event: [:0]const u8) !bool {
    ...
    if (!try L.Lgetmetafield(idx, event))
        return false;
    L.pushvalue(idx);
...
pub fn Ztolstringk(L: *lua.State, idx: i32) ![]const u8 {
    ...
    switch (L.typeOf(idx)) {
        ...
        .Boolean => try L.pushlstring(if (L.toboolean(idx)) "true" else "false"),
        .Number => {
...
fn tag_error(L: *lua.State, narg: i32, tag: lua.Type, comptime msg: ?[]const u8) anyerror {
    ...
    const obj = lapi.Atoobject(L, narg);
...
pub fn Zcheckstack(L: *lua.State, space: usize, msg: ?[]const u8) !void {
...
test Zpushvalue {
    ...
    if (comptime lua.config.VECTOR_SIZE == 3) {
        try Zpushvalue(L, @Vector(3, f32){ 1.0, 2.0, 3.0 });
    } else {
        try Zpushvalue(L, @Vector(4, f32){ 1.0, 2.0, 3.0, 4.0 });
    }
    {
        defer L.pop(1);
...
test Zcheckvalue {
    ...
    try Zpushvalue(L, @as(structDefaults, .{ .x = 1, .y = 2 }));
    ...
    try Zpushvalue(L, @as(structDefaults2, .{ .x = 1, .y = 2 }));
...
test Zsetfield {
    ...
    {
        try Zsetfield(L, lua.GLOBALSINDEX, "a", 455);
        try Zsetfield(L, lua.GLOBALSINDEX, "b", "str");
        try Zsetfield(L, lua.GLOBALSINDEX, "c", true);
    ...
    {
        try Zsetfield(L, lua.REGISTRYINDEX, "a", 455);
        try Zsetfield(L, lua.REGISTRYINDEX, "b", "str");
        try Zsetfield(L, lua.REGISTRYINDEX, "c", true);
...
test Zsetglobal {
    ...
    try Zsetglobal(L, "a", 455);
    try Zsetglobal(L, "b", "str");
    try Zsetglobal(L, "c", true);
